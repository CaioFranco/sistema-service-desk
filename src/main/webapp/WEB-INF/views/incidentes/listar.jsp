...
<body>
	...
	<header id="product-highlight" class="clearfix">
			<div id="product-overview" class="container">
				<h1 class="product-title">Incidentes Registrados</h1>
				<p> ${sucesso} </p>
			</div>
	</header>
	<div class="container">
		<table width="100%">
		    ...
		    <c:forEach items="${incidentes}" var="incidente">
		        <tr>
		            <td>${incidente.id}</td>
		            <td>${incidente.idCliente}</td>
		            <td>${incidente.idAtendente}</td>
		            <td>${incidente.descricao}</td>
		            <td>${incidente.status}</td>
		            <td><fmt:formatDate pattern="dd/MM/yyyy" value = "${incidente.creationTime}"/></td>
		            <td><a href="${s:mvcUrl('IC#encerrar').arg(0, incidente.id).build() }">Encerrar</a></td>
		        </tr>
		    </c:forEach>
		</table>
		<br>
